...
U=14;D=34;OK=24;BACK=22;EXE=95;AC=12
TXT=(20,24,30);MUT=(105,110,120);SEL=(35,115,220);ERR=(210,55,55);BG=(255,255,255)

ONE=(("BLOCK","Block"),("FLAPPY","Flappy"),("MINESWEEPER","Mine"),("PAC-MAN","Pacman"),("SNAKE","Snake"),("SUDOKU","sudoku"),("TETRIS","Tetris2"))
TWO=(("CHESS","Chess"),("CONNECT 4","Connect4"),("TIC-TAC-TOE","TicTac"),("WORDLE","Wordle"))
UTIL=(("TRIANGLE SOLVER","Triangle"),("KEY TEST","KeyTest"))
P=((("GAMES",1),("UTILITIES",4)),(("1 PLAYER",2),("2 PLAYER",3)),ONE,TWO,UTIL)
T=("GAME EXPLORER","GAMES","1 PLAYER GAMES","2 PLAYER GAMES","UTILITIES")
UP=(0,0,1,1,0);LK=None;LOAD={}
...
def page(p,s):
 ...
 elif p==2:draw_string(334,9,str(len(ONE)),MUT,"small")
 elif p==3:draw_string(334,9,str(len(TWO)),MUT,"small")
 i=0
 while i<len(P[p]):
  a=P[p][i];y=41+i*19
 ...
 draw_string(274,181,"BACK  AC EXIT",MUT,"small")
 cur(s,SEL);show_screen()

def err(s):
 clear_screen()
 draw_string(18,54,"COULD NOT OPEN",ERR,"medium")
 draw_string(18,82,s[:52],TXT,"small")
 draw_string(18,116,"PRESS BACK",MUT,"small")
 show_screen()
 while getkey()!=None:pass
 while getkey()!=BACK:pass
 while getkey()!=None:pass
...
def main():
 ...
 while 1:
  ...
  if k==U and s:s-=1
   ...
   if p==0:return
   p=UP[p];s=0;page(p,s);continue
  elif k==OK or k==EXE:
   if p<2:
    p=P[p][s][1];s=0;page(p,s);continue
   run(P[p][s]);page(p,s);continue
  if s!=old:
   cur(old,BG);cur(s,SEL);show_screen()
main()
